...
export const GET = withRouteHandler(
  async (req: NextRequest, { params }: { params: Promise<{ invitationId: string }> }) => {
    ...
    if (!session?.user?.id) {
      if (isAcceptFlow) {
        return NextResponse.redirect(
          new URL(`/invite/${invitationId}?token=${token}`, getBaseUrl())
        )
      }
    ...
    try {
      ...
      if (!isInvitee) {
        const hasAdminAccess = await hasWorkspaceAdminAccess(
          session.user.id,
...
export const DELETE = withRouteHandler(
  async (_request: NextRequest, { params }: { params: Promise<{ invitationId: string }> }) => {
    const { invitationId } = await params
    const session = await getSession()
