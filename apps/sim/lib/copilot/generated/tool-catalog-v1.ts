  ...
  parameters: {
    ...
    properties: {
      operation: {
        ...
        enum: ['create', 'append', 'update', 'patch', 'rename', 'delete'],
      },
      target: {
        type: 'object',
        description:
          'Explicit file target. Use kind=new_file + fileName for create. Use kind=file_id + fileId for append, update, patch, rename, and delete. Emit target keys in this order: kind, fileId, fileName.',
        properties: {
          kind: {
            type: 'string',
      ...
      edit: {
        type: 'object',
        description:
          'Patch metadata. Use strategy=search_replace for exact text replacement, or strategy=anchored for line-based inserts/replacements/deletions. Emit edit keys in this order: strategy, search, replace, replaceAll, mode, occurrence, before_anchor, after_anchor, anchor, start_anchor, end_anchor, content.',
        properties: {
          strategy: {
            type: 'string',
          ...
          content: {
            type: 'string',
            description:
              'Inserted or replacement content for anchored edits. Not used for delete_between.',
          },
        },
      },
      newName: {
      ...
      content: {
        type: 'string',
        description:
          'File content for create, append, or update. For .pptx/.docx/.pdf this must be JavaScript source code for the corresponding generator runtime.',
      },
    },
